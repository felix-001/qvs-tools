...
var (
	AllRootNodesMapByAreaIsp map[string][]*localPublic.DynamicRootNode
...
func GetNodeByIp() {
	for _, node := range AllNodesMap {
		for _, ipInfo := range node.Ips {
			if ipInfo.Ip == Conf.Ip {
				_, ok := AllRootNodesMapByNodeId[node.Id]
...
func loadStaticNodes() []StaticNode {
	var nodes struct {
	...
	if err := qconfig.LoadFile(&nodes, Conf.F); err != nil {
...
func getQvsStaticNodes() {
	rawNodes := loadStaticNodes()
	...
	probeStaticNodeTalk(result)
}

func probeStaticNodeTalk(nodes []*StaticNode) {
