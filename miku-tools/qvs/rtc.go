...
import (
	...
	"middle-source-analysis/staging"
...
func rtcMemLeakTest() {
	for i := 0; i < 100; i++ {
		go singleRtcMemLeakTest(i)
	}
	time.Sleep(30 * time.Second)
}

func singleRtcMemLeakTest(i int) {
	sessionId := startRtcPlay(i)
	// 1. 创建UDP连接
...
func startRtcPlay(i int) string {
	clientIp := staging.ClientInfo{
		OSName:         "Mac OS",
		OSVersion:      "10.15.7",
		BrowserName:    "Chrome",
